...
const formatChar = (date: Date) => (char: string): string | number => {
  ...
  // o - ISO-8601 week-numbering year. This has the same value as Y, except that if the ISO week number (W)
  ...
  if (char === "h") {
    ...
    return adjustedHour < 10 ? "0" + adjustedHour : adjustedHour;
  }
  ...
  // u - Microseconds.2.2). Note that date() will always generate 000000 since it takes an
  // integer parameter, whereas DateTime::format() does support microseconds if DateTime
  // was created with microseconds. Example: 654321
  if (char === "u") {
  ...
  // Z - Timezone offset in seconds. The offset for timezones west of UTC is always negative, and for
